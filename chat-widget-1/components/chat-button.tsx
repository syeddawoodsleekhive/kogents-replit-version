...
export default function ChatButton({
  ...
  useEffect(() => {
    ...
    announce(
      isOpen
        ? t('widget.accessibility.chatClosed')
        : t('widget.accessibility.chatOpened')
    );
  ...
  useEffect(() => {
    announce(
      disabled
        ? t('widget.accessibility.chatButtonDisabled')
        : t('widget.accessibility.chatButtonEnabled')
    );
  }, [disabled, announce, t]);
  ...
  return (
    <>
      ...
      <button
        ...
        disabled={disabled}
        className={cn(
          "flex items-center justify-center rounded-full w-14 h-14 shadow-lg transition-all duration-300 relative chat-button motion-reduce animation-optimize",
          isOpen
        ...
        onFocus={() => {
          announce(
            isOpen
              ? `${t('widget.accessibility.chatButtonFocused')}, ${t(
                  'widget.accessibility.closeChat'
                )}`
              : `${t('widget.accessibility.chatButtonFocused')}, ${t(
                  'widget.accessibility.openChat'
                )}`
          );
        }}
      >
