...
class MessageEncryptionManager {
  ...
  async encryptMessage(content: string, options: MessageEncryptionOptions): Promise<EncryptedMessage> {
    try {
      ...
      const encryptionMetrics = this.calculateEncryptionMetrics(content.length, encryptedContent.length);
  ...
  /**
   * Helper: Analyze content characteristics
   */
  private analyzeContent(content: string): {
    ...
    if (content.length > 100 || wordCount > 20) complexity = "complex";
    else if (content.length > 50 || wordCount > 10) complexity = "medium";
  ...
  /**
   * Helper: Calculate encryption metrics
   */
  private calculateEncryptionMetrics(originalLength: number, encryptedLength: number): {
    expansionRatio: string;
    efficiency: string;
    overhead: number;
  } {
    const expansionRatio = originalLength > 0 ? ((encryptedLength - originalLength) / originalLength * 100).toFixed(2) + "%" : "0%";
    const efficiency = originalLength > 0 ? (originalLength / encryptedLength * 100).toFixed(2) + "%" : "0%";
    const overhead = encryptedLength - originalLength;
